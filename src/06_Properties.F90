...
#include <petsc/finclude/petscdmda.h90>
    ...
    CALL GetPptZoneID(Gmtry,PptFld%ZoneID,PptFld%DefinedByPptZones,  &
        & ierr)
...
#include <petsc/finclude/petscdmda.h90>
    ...
    CHARACTER(LEN=200)                  :: InputFilePptByZone,Route, &
                                         & ViewName,InputDir,EventName
    PetscBool                           :: InputFilePptByZoneFlg,    &
                                         & HomogeneusPptFlg,Verbose
    ...
    CALL GetInputFilePptByZone(InputFilePptByZone,                   &
    ...
    IF (InputFilePptByZoneFlg.AND.(.NOT.HomogeneusPptFlg)) THEN
        ...
        CALL MPI_Comm_rank(MPI_COMM_WORLD,process,ierr)
            ...
            IF (process.EQ.0) THEN
                ...
                Route=ADJUSTL(TRIM(InputDir)//                       &
                    & TRIM(InputFilePptByZone))
                ...
                DO i=1,widthG(1)*widthG(2)*widthG(3)
                    ...
                    IF (ValI.LE.0) THEN
                        CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,&
                            & "[ERROR] -Input_file_ppt_by_zones en"//&
                            & "try only can contain natural number"//&
                            & "s.\n",ierr)
                        STOP
                    END IF
                    ValR=REAL(ValI)
                    CALL VecSetValue(PptZoneID_Global,i-1,ValR,      &
                        & INSERT_VALUES,ierr)
                END DO
                CLOSE(u)
        ...
        CALL VecApplicationToPetsc(Gmtry%DataMngr,PptZoneID_Global,  &
            & ierr)
        ...
        CALL DMGlobalToLocalBegin(Gmtry%DataMngr,PptZoneID_Global,   &
            & INSERT_VALUES,PptZoneID_Local,ierr)
        CALL DMGlobalToLocalEnd(Gmtry%DataMngr,PptZoneID_Global,     &
            & INSERT_VALUES,PptZoneID_Local,ierr)
    ...
    ELSEIF (HomogeneusPptFlg) THEN
        ...
        IF (InputFilePptByZoneFlg) THEN 
            IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,&
                & "[GetProrperties Event] WARNING: -Input_file_ppt"//&
                & "_by_zones and -Homogeneuos_ppt used at the same"//&
                & " time; properties was set homogeneuos in the wh"//&
                & "ole domain.\n",ierr)
        END IF
    END IF
...
SUBROUTINE GetConductivity(Gmtry,PptFld,Cvt,ierr)
    ...
    USE ANISOFLOW_Types,     ONLY : Geometry,PropertiesField,        &
                                  & ConductivityField,InputTypeVar
...
#include <petsc/finclude/petscsys.h>
    ...
    ELSE
        CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,               &
            & "[ERROR] Conductivity InputType invalid.\n",ierr)
        STOP
    END IF
...
#include <petsc/finclude/petscdmda.h90>
    ...
    PetscMPIInt                         :: process
    PetscReal                           :: ValR,one=1.D0
    CHARACTER(LEN=200)                  :: InputFilePptByZone,       &
                                         & InputFileCvtByZone,Route, &
                                         & ViewName,InputDir,EventName
    PetscBool                           :: InputFilePptByZoneFlg,    &
                                         & InputFileCvtByZoneFlg,    &
                                         & HomogeneusPptFlg,         &
                                         & HomogeneusCvtFlg,Verbose
    ...
    CALL GetInputFilePptByZone(InputFilePptByZone,                   &
        & InputFilePptByZoneFlg,ierr)
    CALL GetInputFileCvtByZone(InputFileCvtByZone,                   &
    ...
    IF (InputFileCvtByZoneFlg.AND.(.NOT.HomogeneusCvtFlg)) THEN
        ...
        CALL DMCreateGlobalVector(Gmtry%DataMngr,CvtZoneID_Global,   &
            & ierr)
        ...
        CALL MPI_Comm_rank(MPI_COMM_WORLD,process,ierr)
            IF (process.EQ.0) THEN
                Route=ADJUSTL(TRIM(InputDir)//                       &
                    & TRIM(InputFileCvtByZone))
                OPEN(u,FILE=TRIM(Route),STATUS='OLD',ACTION='READ')
                DO i=1,widthG(1)*widthG(2)*widthG(3)
                    READ(u,*)ValI
                    IF (ValI.LE.0) THEN
                        CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,&
                            & "[ERROR] Input_file_cvt_by_zones ent"//&
                            & "ry only can contain natural numbers"//&
                            & ".\n",ierr)
                        STOP
                    END IF
                    ValR=REAL(ValI)
                    CALL VecSetValue(CvtZoneID_Global,i-1,ValR,      &
                        & INSERT_VALUES,ierr)
                END DO
                CLOSE(u)
            END IF
        ...
        CALL VecApplicationToPetsc(Gmtry%DataMngr,CvtZoneID_Global,  &
            & ierr)

        ViewName="CvtZoneID"
        EventName="GetCvtZoneID"
        CALL ViewConductivity(CvtZoneID_Global,ViewName,EventName,   &
            & ierr)

        CALL DMGlobalToLocalBegin(Gmtry%DataMngr,CvtZoneID_Global,   &
            & INSERT_VALUES,CvtZoneID_Local,ierr)
        CALL DMGlobalToLocalEnd(Gmtry%DataMngr,CvtZoneID_Global,     &
            & INSERT_VALUES,CvtZoneID_Local,ierr)
    ...
    ELSEIF (HomogeneusCvtFlg) THEN
        ...
        IF (InputFileCvtByZoneFlg) THEN 
            IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,&
                & "[GetProrperties Event] WARNING: -Input_file_cvt"//&
                & "_by_zones and -Homogeneuos_cvt used at the same"//&
                & " time; conductivities was set homogeneuos in th"//&
                & "e whole domain.\n",ierr)
        END IF
    ...
    ELSE
        CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,               &
            & "[ERROR] -Input_type_cvt 1 must be used with -Input_"//&
            & "file_(cvt,ppt)_by_zones, which contains the conduct"//&
            & "ivities zonification, or with -Homogeneuos_(cvt,ppt"//&
            & ") 1, which sets the first property of -Input_file_("//&
            & "cvt,ppt) to the whole domain.\n",ierr)
        STOP
    END IF
...
SUBROUTINE GetConductivity_1(Gmtry,PptFld,Cvt,ierr)
    ...
    USE ANISOFLOW_Types,     ONLY : Geometry,ConductivityField,      &
                                  & PropertiesField,TargetFullTensor
    USE ANISOFLOW_Interface, ONLY : GetVerbose,GetInputDir,          &
                                  & GetInputFileCvt
    USE ANISOFLOW_Geometry,  ONLY : VecApplicationToPetsc
...
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetConductivity Event] Conductivity Field is stored as"//&
!         & " Zones by Block\n",ierr)
    ...
    IF (process.EQ.0) THEN
        ...
        DO i=1,CvtLen
            ...
            ELSEIF (ADJUSTL(TRIM(CvtKind)).EQ."k anisotropic") THEN 
                ...
                Cvt%Zone(i)%xz=0.D0
                Cvt%Zone(i)%yz=0.D0
            ELSEIF (ADJUSTL(TRIM(CvtKind)).EQ."k non-orthogonal an"//&
                & "isotropic") THEN 
                READ(u,*)Cvt%Zone(i)%xx,Cvt%Zone(i)%yy,              &
                    & Cvt%Zone(i)%zz,Cvt%Zone(i)%xy,Cvt%Zone(i)%xz,  &
                    & Cvt%Zone(i)%yz
            ELSE
                CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,       &
                    & "[ERROR] File of conductivity properties is "//&
                    & "invalid.\n"&
                    & ,ierr)
                STOP            
            END IF
    ...
    CALL MPI_Bcast(Cvt%Zone(:)%xx,CvtLen,MPI_DOUBLE,0,               &
        & PETSC_COMM_WORLD,ierr)
    CALL MPI_Bcast(Cvt%Zone(:)%yy,CvtLen,MPI_DOUBLE,0,               &
        & PETSC_COMM_WORLD,ierr)
    CALL MPI_Bcast(Cvt%Zone(:)%zz,CvtLen,MPI_DOUBLE,0,               &
        & PETSC_COMM_WORLD,ierr)
    CALL MPI_Bcast(Cvt%Zone(:)%xy,CvtLen,MPI_DOUBLE,0,               &
        & PETSC_COMM_WORLD,ierr)
    CALL MPI_Bcast(Cvt%Zone(:)%xz,CvtLen,MPI_DOUBLE,0,               &
        & PETSC_COMM_WORLD,ierr)
    CALL MPI_Bcast(Cvt%Zone(:)%yz,CvtLen,MPI_DOUBLE,0,               &
        & PETSC_COMM_WORLD,ierr)
...
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetConductivity Event] Conductivity Field was satisfac"//&
!         & "torily created\n",ierr)
...
SUBROUTINE GetConductivity_2(Gmtry,PptFld,Cvt,ierr)
    ...
    USE ANISOFLOW_Types,     ONLY : Geometry,PropertiesField,        &
                                  & ConductivityField,TargPetscVec
    USE ANISOFLOW_Interface, ONLY : GetVerbose,GetInputDir,          &
                                  & GetInputFileCvt
    USE ANISOFLOW_View,      ONLY : ViewConductivity
...
#include <petsc/finclude/petscdmda.h90>
    ...
    Cvt%DefinedBy=3
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetProrperties Event] Conductivity Field is stored by "//&
!         & "Block\n",ierr)
    ...
    IF (process.EQ.0) THEN
        ...
        DO i=1,widthG(1)*widthG(2)*widthG(3)
            READ(u,*)ValR
            CALL VecSetValue(Cvt_xx_Global,i-1,ValR,INSERT_VALUES,   &
                & ierr)
        END DO
        CLOSE(u)
    END IF
    ...
    CALL DMGlobalToLocalBegin(Gmtry%DataMngr,Cvt_xx_Global,          &
        & INSERT_VALUES,Cvt%xx,ierr)
    CALL DMGlobalToLocalEnd(Gmtry%DataMngr,Cvt_xx_Global,            &
        & INSERT_VALUES,Cvt%xx,ierr)
    ...
    ! Saving a vector with zeros (If we put a conditionals on the 
    ! stencil it can be avoided, but would be unclear)
...
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetProrperties Stage] Conductivity Field was satisfact"//&
!         & "orily created\n",ierr)
...
!         & "Block\n",ierr)
    ...
    IF (process.EQ.0) THEN
        ...
        DO i=1,widthG(1)*widthG(2)*widthG(3)
            ...
            CALL VecSetValue(Cvt_xx_Global,i-1,ValR(1),INSERT_VALUES,&
                & ierr)
            CALL VecSetValue(Cvt_yy_Global,i-1,ValR(2),INSERT_VALUES,&
                & ierr)
            CALL VecSetValue(Cvt_zz_Global,i-1,ValR(3),INSERT_VALUES,&
                & ierr)
        END DO
        CLOSE(u)
    END IF
    ...
    CALL DMGlobalToLocalBegin(Gmtry%DataMngr,Cvt_yy_Global,          &
        & INSERT_VALUES,Cvt%yy,ierr)
    CALL DMGlobalToLocalEnd(Gmtry%DataMngr,Cvt_yy_Global,            &
        & INSERT_VALUES,Cvt%yy,ierr)
    CALL VecDestroy(Cvt_yy_Global,ierr)

    ViewName="Cvt_zz"
    CALL ViewConductivity(Cvt%zz,ViewName,EventName,ierr)
    CALL DMGlobalToLocalBegin(Gmtry%DataMngr,Cvt_zz_Global,          &
        & INSERT_VALUES,Cvt%zz,ierr)
    CALL DMGlobalToLocalEnd(Gmtry%DataMngr,Cvt_zz_Global,            &
        & INSERT_VALUES,Cvt%zz,ierr)
...
#include <petsc/finclude/petscdmda.h90>
    ...
    IF (process.EQ.0) THEN
        ...
        DO i=1,widthG(1)*widthG(2)*widthG(3)
            ...
            CALL VecSetValue(Cvt_zz_Global,i-1,ValR(3),INSERT_VALUES,&
                & ierr)
            CALL VecSetValue(Cvt_xy_Global,i-1,ValR(4),INSERT_VALUES,&
                & ierr)
            CALL VecSetValue(Cvt_xz_Global,i-1,ValR(5),INSERT_VALUES,&
                & ierr)
            CALL VecSetValue(Cvt_yz_Global,i-1,ValR(6),INSERT_VALUES,&
                & ierr)
        END DO
        CLOSE(u)
    END IF
    ...
    CALL DMGlobalToLocalBegin(Gmtry%DataMngr,Cvt_xy_Global,          &
        & INSERT_VALUES,Cvt%xy,ierr)
    CALL DMGlobalToLocalEnd(Gmtry%DataMngr,Cvt_xy_Global,            &
        & INSERT_VALUES,Cvt%xy,ierr)
    CALL VecDestroy(Cvt_xy_Global,ierr)

    ViewName="Cvt_xz"
    CALL ViewConductivity(Cvt%xz,ViewName,EventName,ierr)
    CALL DMGlobalToLocalBegin(Gmtry%DataMngr,Cvt_xz_Global,          &
        & INSERT_VALUES,Cvt%xz,ierr)
    CALL DMGlobalToLocalEnd(Gmtry%DataMngr,Cvt_xz_Global,            &
        & INSERT_VALUES,Cvt%xz,ierr)
    CALL VecDestroy(Cvt_xz_Global,ierr)

    ViewName="Cvt_yz"
    CALL ViewConductivity(Cvt%yz,ViewName,EventName,ierr)
    CALL DMGlobalToLocalBegin(Gmtry%DataMngr,Cvt_yz_Global,          &
        & INSERT_VALUES,Cvt%yz,ierr)
    CALL DMGlobalToLocalEnd(Gmtry%DataMngr,Cvt_yz_Global,            &
        & INSERT_VALUES,Cvt%yz,ierr)
...
SUBROUTINE GetStorage(Gmtry,PptFld,Sto,ierr)
    ...
    USE ANISOFLOW_Types,     ONLY : Geometry,PropertiesField,        &
                                  & SpecificStorageField,InputTypeVar
    USE ANISOFLOW_Interface, ONLY : GetVerbose,GetInputType
...
#include <petsc/finclude/petscdmda.h90>
    ...
    PetscReal                           :: ValR,one=1.D0
    CHARACTER(LEN=200)                  :: InputFilePptByZone,       &
                                         & InputFileStoByZone,Route, &
                                         & ViewName,InputDir,EventName
    PetscBool                           :: InputFilePptByZoneFlg,    &
                                         & InputFileStoByZoneFlg,    &
                                         & HomogeneusPptFlg,         &
                                         & HomogeneusStoFlg,Verbose
    Vec,POINTER                         :: ZoneID_tmp
    ...
    CALL GetInputFilePptByZone(InputFilePptByZone,                   &
        & InputFilePptByZoneFlg,ierr)
    CALL GetInputFileStoByZone(InputFileStoByZone,                   &
    ...
    IF (InputFileStoByZoneFlg.AND.(.NOT.HomogeneusStoFlg)) THEN
        ...
        CALL DMCreateGlobalVector(Gmtry%DataMngr,StoZoneID_Global,   &
            & ierr)
        ...
        CALL MPI_Comm_rank(MPI_COMM_WORLD,process,ierr)
            IF (process.EQ.0) THEN
                ...
                DO i=1,widthG(1)*widthG(2)*widthG(3)
                    ...
                    IF (ValI.LE.0) THEN
                        CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,&
                            & "[ERROR] Input_file_sto_by_zones ent"//&
                            & "ry only can contain natural numbers"//&
                            & ".\n",ierr)
                        STOP
                    END IF
                    ValR=REAL(ValI)
                    CALL VecSetValue(StoZoneID_Local,i-1,ValR,       &
                        & INSERT_VALUES,ierr)
                END DO
                CLOSE(u)
            END IF
        ...
        CALL VecApplicationToPetsc(Gmtry%DataMngr,StoZoneID_Local,   &
            & ierr)
        ...
        CALL DMGlobalToLocalBegin(Gmtry%DataMngr,StoZoneID_Global,   &
            &INSERT_VALUES,StoZoneID_Local,ierr)
        CALL DMGlobalToLocalEnd(Gmtry%DataMngr,StoZoneID_Global,     &
            & INSERT_VALUES,StoZoneID_Local,ierr)
    ...
    ELSEIF (HomogeneusStoFlg) THEN
        ...
        CALL VecApplicationToPetsc(Gmtry%DataMngr,StoZoneID_Global,  &
            & ierr)
        ...
        IF (InputFileStoByZoneFlg) THEN 
            IF (Verbose)                                             &
                & CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,     &
                & "[GetProrperties Event] WARNING: -Input_file_sto"//&
                & "_by_zones and -Homogeneuos_sto used at the same"//&
                & " time; specific storages was set homogeneuos in"//&
                & " the whole domain.\n",ierr)
        END IF
    ELSEIF (InputFilePptByZoneFlg.OR.HomogeneusPptFlg) THEN
        ZoneID_tmp => TargPetscVec(PptFld%ZoneID)
    ...
    ELSE
        CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,               &
            & "[ERROR] Input_type_sto must use Input_file_sto_by_z"//&
            &"ones or Input_file_ppt_by_zones containing the zonif"//&
            & "ication of specific storage.\n",ierr)
        STOP
    END IF
...
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetStorage Event] Storage Field is stored as Zones by "//&
!         & "Block\n",ierr)
    ...
    CALL MPI_Bcast(Zone_tmp(:),StoLen,MPI_DOUBLE,0,PETSC_COMM_WORLD, &
        & ierr)

    CALL VecCreateSeq(PETSC_COMM_SELF,StoLen,Sto%Zone,ierr)

    CALL VecSetValues(Sto%Zone,StoLen,(/(i,i=0,StoLen-1)/),Zone_tmp, &
        & INSERT_VALUES,ierr)
...
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetStorage Event] Storage Field was satisfactorily crE"//&
!         & "ated\n",ierr)
...
#include <petsc/finclude/petscdmda.h90>
    ...
    PetscReal,POINTER       :: TmpStoZone(:),TmpStoZoneID3D(:,:,:),  &
                             & TmpStoZoneID2D(:,:),                  &
                             & TmpStoCell3D(:,:,:),TmpStoCell2D(:,:)
    ...
    IF (.NOT.((Sto%DefinedBy.EQ.1).OR.(Sto%DefinedBy.EQ.2))) THEN
        PRINT*,"ERROR: To use StorageZoneToCell subroutine the spe"//&
            & "cific storage has to be definde by zone."
    END IF
    ...
    IF (widthG(3).NE.1) THEN
        CALL DMDAVecGetArrayReadF90(Gmtry%DataMngr,Sto%ZoneID,       &
            & TmpStoZoneID3D,ierr)
        CALL DMDAVecGetArrayReadF90(Gmtry%DataMngr,Sto%Cell,         &
            & TmpStoCell3D,ierr)
    ELSE
        CALL DMDAVecGetArrayReadF90(Gmtry%DataMngr,Sto%ZoneID,       &
            & TmpStoZoneID2D,ierr)
    ...
    IF (widthG(3).NE.1) THEN
        CALL DMDAVecRestoreArrayReadF90(Gmtry%DataMngr,Sto%ZoneID,   &
            & TmpStoZoneID3D,ierr)
        CALL DMDAVecRestoreArrayReadF90(Gmtry%DataMngr,Sto%Cell,     &
            & TmpStoCell3D,ierr)
    ELSE
        CALL DMDAVecRestoreArrayReadF90(Gmtry%DataMngr,Sto%ZoneID,   &
            & TmpStoZoneID2D,ierr)
    ...
    ELSE IF (Sto%DefinedBy.EQ.2) THEN
        ! NULLIFY(Sto%ZoneID) ! Because I used a very tricky way to 
        ! use it as a pointer, I can't nullify it as a usual pointer 
        ! but it's supposed it doesn't will get in troubles the code.
        Sto%DefinedBy=0
    END IF
...
#include <petsc/finclude/petscdmda.h90>
    ...
    Sto%DefinedBy=3
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         &"[GetProrperties Event] Storage Field is stored by Block.\n"&
!         & ,ierr)
...
!     IF (Verbose) CALL PetscSynchronizedPrintf(PETSC_COMM_WORLD,      &
!         & "[GetProrperties Stage] Storage Field was satisfactorily"//&
!         & " created\n",ierr)
...
#include <petsc/finclude/petscdmda.h90>
    ...
    IF (Ppt%StnclType.EQ.1) THEN          ! Stencil type star
        IF ((PptFld%Cvt%DefinedBy.EQ.1).OR.                          &
            & (PptFld%Cvt%DefinedBy.EQ.2)) THEN 
            ! Defined by zones, so have to get the information from 
            ! Zone(ZoneID(i))
            IF ((Ppt%StnclTplgy(CenterID).EQ.1).OR.                  &
                & (Ppt%StnclTplgy(CenterID).EQ.3).OR.                &
                & (Ppt%StnclTplgy(CenterID).EQ.4)) THEN 
                ! Only get properties for active blocks
                IF (widthG(3).NE.1) THEN ! 3D mesh
                    
                    CALL DMDAVecGetArrayReadF90(Gmtry%DataMngr,      &
                        & PptFld%Cvt%ZoneID,TmpCvt3D,ierr)
                    ...
                    CALL DMDAVecRestoreArrayReadF90(Gmtry%DataMngr,  &  
                        & PptFld%Cvt%ZoneID,TmpCvt3D,ierr)
                    Ppt%Cvt(:)=PptFld%Cvt%Zone(ValI(:))
                ELSE ! 2D mesh
                    CALL DMDAVecGetArrayReadF90(Gmtry%DataMngr,      &
                        & PptFld%Cvt%ZoneID,TmpCvt2D,ierr)
        ...
        ELSE IF (Pptfld%Cvt%DefinedBy.EQ.3) THEN
            ...
            END IF  
        ELSE
            PRINT*,"ERROR: The properties has to be defined by cel"//&
                & "l or by zone"
            STOP
        END IF
    ...
    ELSEIF (Ppt%StnclType.EQ.2) THEN          ! Stencil type box (19)
        ...
        IF (j.EQ.0) THEN
            Ppt%Cvt(1)=TensorZero
            Ppt%Cvt(6:8)=TensorZero
            Ppt%Cvt(15)=TensorZero
        END IF
        IF (i.EQ.0) THEN
            Ppt%Cvt(2)=TensorZero
            Ppt%Cvt(6)=TensorZero
            Ppt%Cvt(9)=TensorZero
            Ppt%Cvt(12)=TensorZero
            Ppt%Cvt(16)=TensorZero
        END IF
        IF (i.EQ.(widthG(1)-1)) THEN
            Ppt%Cvt(4)=TensorZero
            Ppt%Cvt(8)=TensorZero
            Ppt%Cvt(11)=TensorZero
            Ppt%Cvt(14)=TensorZero
            Ppt%Cvt(18)=TensorZero
        END IF
        IF (j.EQ.(widthG(2)-1)) THEN
            Ppt%Cvt(5)=TensorZero
            Ppt%Cvt(12:14)=TensorZero
            Ppt%Cvt(19)=TensorZero
        END IF
...
!         IF (ALLOCATED(PptFld%Cvt%CvtZone))                           &
!             & DEALLOCATE(PptFld%Cvt%CvtZone)
